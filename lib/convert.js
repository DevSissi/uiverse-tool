...
// Inside these elements every space and newline is significant, so their text
// must not pick up the pretty-printer's indentation.
const PREFORMATTED_ELEMENTS = new Set(['pre', 'textarea', 'listing', 'plaintext']);
...
// Newlines are encoded so a multi-line value (an SVG `path d`, a wrapped
// `class`) stays on one line: JSX decodes the entity back, whereas raw
// newlines would pick up the surrounding indentation and change the value.
...
const serializeNode = (node, preformatted = false) => {
  if (node.nodeName === '#text') {
    const text = escapeText(node.value);
    // JSX decodes the entity, so encoding keeps preformatted text exact even
    // after the output is indented.
    return preformatted ? text.replace(/\r/g, '&#13;').replace(/\n/g, '&#10;') : text;
  }
  ...
  const inPre = preformatted || PREFORMATTED_ELEMENTS.has(tag);
  const children = (node.childNodes || []).map((child) => serializeNode(child, inPre)).join('');
